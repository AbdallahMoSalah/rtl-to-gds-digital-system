...
set_clock_transition $CLK_TRANS [get_clocks $CLK1_NAME]
...
set_clock_transition $CLK_TRANS [get_clocks $CLK2_NAME]
...
create_generated_clock -master_clock $CLK1_NAME -source [get_ports REF_CLK] \
                       -name "ALU_CLK" [get_port clk_gate_unit/GATED_CLK] \
                       -divide_by 1
...
set_input_delay $in_delay -clock UART_RX_CLK [get_port RX_IN]

#Constrain Output Paths
set_output_delay $out_delay -clock UART_TX_CLK [get_port TX_OUT]

#Constrain Scan Input Paths
set_input_delay $in2_delay -clock $CLK3_NAME [get_port {test_mode SI SE}]
...
####################################################################################
           ...
           #########################################################
####################################################################################
set_driving_cell -library scmetro_tsmc_cl013g_rvt_ss_1p08v_125c -lib_cell BUFX2M -pin Y [get_port {RX_IN SI SE test_mode}]
...
####################################################################################
           ...
           #########################################################
####################################################################################
set_load 0.1 [get_port {TX_OUT SO}]
